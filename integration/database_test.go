...
import (
	...
	appdb "github.com/lihongjie0209/search-service/internal/database"
	"github.com/lihongjie0209/search-service/internal/migration"
...
func TestRepositoryAndMigrations(t *testing.T) {
	for _, databaseType := range []string{"postgres", "mysql"} {
		t.Run(databaseType, func(t *testing.T) {
			...
			var inboxTables int
			if databaseType == "postgres" {
				if err := db.GetContext(ctx, &inboxTables, `SELECT count(*) FROM pg_tables WHERE schemaname = current_schema() AND tablename = 'search_event_inbox'`); err != nil {
					t.Fatal(err)
				}
				var timezone string
			...
			} else if err := db.GetContext(ctx, &inboxTables, `SELECT count(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'search_event_inbox'`); err != nil {
				t.Fatal(err)
			}
			if inboxTables != 1 {
				t.Fatal("search Inbox table was not created")
			}
