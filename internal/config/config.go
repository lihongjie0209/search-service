...
type Config struct {
	...
	Projection    Projection    `mapstructure:"projection"`
}
...
type Health struct {
	...
	OpenSearchTimeout time.Duration `mapstructure:"opensearch_timeout"`
}
...
func setDefaults(v *viper.Viper) {
	...
	v.SetDefault("database.schema", "search")
	...
	v.SetDefault("health.opensearch_timeout", "2s")
	...
	v.SetDefault("jwt.issuer", "search-service")
	...
	v.SetDefault("auth.audience", "search-service")
	...
	v.SetDefault("opensearch.index", "platform-search-v1")
	v.SetDefault("opensearch.timeout", "5s")
	v.SetDefault("opensearch.insecure_skip_verify", false)
	v.SetDefault("projection.durable", "search-document-projection")
	v.SetDefault("projection.subject", "platform.search.document.>")
	...
	v.SetDefault("migration.table", "search_schema_migrations")
...
func (c Config) Validate() error {
	...
	if c.HTTP.RequestTimeout <= 0 || c.Health.DatabaseTimeout <= 0 || c.Health.RedisTimeout <= 0 || c.Health.OpenSearchTimeout <= 0 {
	...
	if c.EventBus.Enabled && (!c.Database.Enabled || c.Projection.Durable == "" || c.Projection.Subject == "") {
		return errors.New("enabled event_bus requires database and projection durable/subject for Inbox idempotency")
	}
	if c.Idempotency.Enabled && (!c.Redis.Enabled || len(c.Idempotency.GRPCMethods) == 0 || c.Idempotency.ProcessingTTL <= 0 || c.Idempotency.ResultTTL <= 0 || c.Idempotency.FailureTTL <= 0) {
		return errors.New("enabled idempotency requires redis, at least one gRPC method pattern, and positive TTL values")
	}
