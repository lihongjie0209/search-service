...
func (s *clientStub) ListBindings(ctx context.Context, request *authorizationv1.ListBindingsRequest, _ ...grpc.CallOption) (*authorizationv1.ListBindingsResponse, error) {
	...
	s.pageSize = request.GetPage().GetPageSize()
...
func TestVisibilityTokensAreServerDerived(t *testing.T) {
	...
	client := &clientStub{}
	tokens, err := New(client).VisibilityTokens(ctx, "tenant-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	if client.pageSize != 100 {
		t.Fatalf("authorization page_size=%d", client.pageSize)
	}
