...
func TestService_Ready(t *testing.T) {
	...
	service := New(nil, client, nil, config.Config{Health: config.Health{DatabaseTimeout: time.Second, RedisTimeout: time.Second, OpenSearchTimeout: time.Second}})
...
func TestService_ReadyWhenRedisIsDown(t *testing.T) {
	...
	defer func() { _ = client.Close() }()
	service := New(nil, client, nil, config.Config{Health: config.Health{DatabaseTimeout: time.Second, RedisTimeout: 10 * time.Millisecond, OpenSearchTimeout: time.Second}})
	status, ready := service.Ready(t.Context())
	if ready {
